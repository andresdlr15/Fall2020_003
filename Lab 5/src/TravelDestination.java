
public class TravelDestination {

	public static void main(String[] args) {
		System.out.println("My name is Heena Rathore. I wish to travel to England once this Covid goes away!");
		
		//Adding print statement for name and hobby
		System.out.println("My name is Julie Ramoz. I enjoy creating digital art in my freetime. :)");
		
		//Adding a print statement with name and hobby
		System.out.println("My name is Tanner McLees. One of my hobbies is skateboarding.");
		
		// console output with name and hobby
		System.out.println("My name is Travis Sauer. Hiking in mountainous areas is one of my favorite activities.");
		
		// console output with name and hobby
		System.out.println("My name is Heila Shahidi. In my pass time I enjoy playing chess and cubing.");
		
		//Adding print statement with name and hobby
		System.out.println("My name is Eric Sananikone. One of my hobbies is running.");
		
		//Adding print statement with name and hobby (As requested in task 2 of lab 5) (omh118)
		System.out.println("My name is Moussa Dastgheib. My favorite hobby is photography.\n");
		
		//Adding print statement with my name and hobby (task2)
		System.out.println("My name is Rishika Someshwar. I like listening to audiobooks.\n");
		
		//Adding print statement with my name and hobby
		System.out.println("My name is Paul O'Bar. I like making games.\n");

		
		//Adding print statement for name and hobby
		System.out.println("My name is Wycliff Lumumba. I like reading books in my freetime\n");
		
		//Task 2: Name and hobby.
		System.out.println("My name is Alan Cabrera and I like to play electric bass.");
		
		//Print statement with my name and hobby
		System.out.println("My name is Kevin Nguyen and I like playing games");
		
		//Print statement with full name and hobby
		System.out.println("My name is Bethany Salazar and I like to explore new trails.");
		
		//Print statement with full name and hobby
		System.out.println("My name is Michael Stepp and I like to play electric guitar");				
    
		//Ading print statement with my name and hobby
		System.out.println("My name is Joshua Romero. I enjoy playing video games and watching anime.\n");
		
		//Added print statement specific to my name and hobby, task 2
		System.out.println("My name is Gideon Reyes, and I love moshing and stage-diving at concerts.");
		
		//Added print statement witn name and hobby
		System.out.println("My name is Vanessa Gerber and one of my hobbies is longboarding");
    
		//print statement with full name and hobby 
		System.out.println("My name is Khalid Farhoud and I like to play football.");
		
		//Print statement with full name and hobby
		System.out.println("My name is Andrew Constancio and I like to skateboard");
		
		//Print statement with full name and hobby

		System.out.println("My name is Aiden Johnson and I like to fish.");
		
		//Print statement with full name and hobby
		System.out.println("My name is Gloria Perez and I like to go on hikes.");
    
		System.out.println("My name is Raisul Islam and I like to travel.");

		System.out.println("My name is Kevin Chan and I like to try new foods");

		//Print statement with full name and hobby
		System.out.println("My name is Seth Gibson and I like to play video games.");

		//Print statement with full name and hobby
		System.out.println("My name is Theresa Crawford and I like to read and eat out at new places.");
		
		//Print statement with full name and hobby
		System.out.println("My name is Taylor Kulawik and I like to hike and workout.");
		
		System.out.println("My name is Ruben Andrade and I like to write code and write stories.");
		
		//Print statement with full name and hobby
		System.out.println("My name is Dillon Dakil and I like to rockclimb, cliff dive and hike.");
		
		System.out.println("My name is Samir Stanislav Yezhnikovsky and I like to read.");
		
		System.out.println("My name is Amber Beserra, one of my hobbies is drawing");
		
		System.out.println("My name is Andres De La Rosa, and I love to ski and snowboard.");
	}

}
